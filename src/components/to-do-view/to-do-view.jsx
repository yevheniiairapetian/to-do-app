...
import copyToDo from './src/copyToDo.mp3';
...
export const ToDoView = () => {
  ...
  function updateListPlaceholder() {
    const hasLists = $(".list-row").children().length > 0;
    $(".no-lists-placeholder").css("display", hasLists ? "none" : "block");
  }
  ...
  function loadLists() {
    ...
    updateListPlaceholder(); // ✅ Now it runs after lists are added
  }
  ...
  function createListElement(title, todos) {
    ...
    // let cloneListButton = $('<button class="clone-list button"><span title="Make a copy" class=""> <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" /></svg></span></button>');
    // div.append(cloneListButton);
  ...
  function createToDoElement(text, completed) {
    ...
    copyButton.on('click', function () {
      ...
      let duplicatedLi = createToDoElement(updatedText, false); // Always unstruck

      // ✨ Add animation
      duplicatedLi.addClass("highlight");
      setTimeout(() => {
        duplicatedLi.removeClass("highlight");
      }, 800);

      // 🔊 Play sound
      const audio = new Audio(copyToDo); // Make sure this file exists
      audio.preload = "auto";
      audio.play();

      li.after(duplicatedLi);
      saveLists();

      // 📝 Status message
      let updateEdits = li.closest(".new-list").find(".update");
      updateEdits.text("To-do duplicated!").show();
      setTimeout(() => {
        updateEdits.text("All your edits are automatically saved").show();
      }, 2800);
  ...
  // Always bind modal buttons once
$(document).ready(function () {
  $("#bulkDeleteAllLists").off("click").on("click", function () {
    $("#exampleModal5").fadeIn(300);
  });

  $("#confirmBulkDeleteListBtn").off("click").on("click", function () {
    $(".new-list").remove();
    localStorage.removeItem("savedLists");

    $(".update").text("All lists deleted.").show();
    setTimeout(() => {
      $(".update").text("All your edits are automatically saved").show();
    }, 2800);

    updateListPlaceholder();
    $("#exampleModal5").fadeOut(300);

    // Show placeholder if no lists remain
    if ($(".new-list").length === 0) {
      $(".no-lists-placeholder").fadeIn(300);
    }
  });

  $("#cancelBulkDeleteListBtn").off("click").on("click", function () {
    $("#exampleModal5").fadeOut(300);
  });

  $(document).ready(function () {
  $("body").on("click", "#cancelBulkDeleteListBtn", function () {
    $("#exampleModal5").fadeOut(300);
  });
});
  ...
  return (
    ...
    <div className="container-fluid app-container">
      ...
      <div className="list-control-buttons">
        ...
        <button tabIndex="5" id="bulkDeleteAllLists" className="danger-button ms-3">
          🗑️ Clear All
        </button>
      </div>
      ...
      <div className="no-lists-placeholder text-center mt-4">
      ...
      <div className="modal" id="exampleModal2" tabIndex="-1" aria-labelledby="deleteConfirmLabel" aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content">
            ...
            <div className="modal-body">
              Are you sure you want to delete all to-dos in this list? It's a permanent action.
            </div>
            <div className="modal-footer">
      ...
      <div className="modal" id="exampleModal4" tabIndex="-1" aria-labelledby="deleteConfirmLabel" aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content">
            ...
            <div className="modal-body">
              Are you sure you want to delete this list? It's a permanent action.
            </div>
            <div className="modal-footer">
              <button type="button" className="btn btn-success" data-bs-dismiss="modal">Cancel</button>
      ...
      <div className="modal" id="exampleModal5" tabIndex="-1" aria-labelledby="deleteConfirmLabel" aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="deleteConfirmLabel">Confirm Deletion</h5>
              <button type="button" className="btn-close" id="cancelBulkDeleteListBtn" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div className="modal-body">
              Are you sure you want to delete all lists? It's a permanent action.
            </div>
            <div className="modal-footer">
              <button type="button" className="btn btn-success" id="cancelBulkDeleteListBtn" data-bs-dismiss="modal">Cancel</button>
              <button type="button" className="btn btn-danger" id="confirmBulkDeleteListBtn">Delete</button>
            </div>
          </div>
        </div>
      </div>
